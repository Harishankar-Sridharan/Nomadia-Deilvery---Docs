...
Note: Slot proposals are influenced by multiple factors, such as user availability, vehicle

availability, and mission opening times. Make sure your slot definitions take these factors into account.

To reserve a slot through the appointment booking portal, follow these steps:

Launch the Nomadia Delivery application and go to the Missions tab.
...
1-star: The proposed slot is more than 30 minutes away from the existing tour plans in the

back office.

2-star: The proposed slot is between 10 and 30 minutes away from the existing tour plans

in the back office.

3-star: The proposed slot is less than 10 minutes away from the existing tour plans in the
...
new tour plan for the selected slot (if no existing tour is scheduled in the back office at that
...
To configure Nomadia Delivery for the appointment booking portal, transporters need to
...
Open the Booking portal tab.

Turn on the toggle Display the booking portal.

Configure the fields you want to display to the end customer to capture details needed for

successful delivery. By default, Name and Address are mandatory, as they are essential for
...
Example: Field name – Access Code (the code the deliverer needs to access the customer’s

premises).

Choose the Mode for the field, such as Input Required, Hidden, Read-only, or Read/Write, to
...
The URL in the code sample automatically redirects to the booking portal and can be
...
The customer can click Schedule to view real-time optimized time slots and then select their
...
application. Once the booking is confirmed:
...
Enable the Reschedule a Delivery toggle to activate the reschedule button in customer email
...
link to be active from a maximum of 12 hours before delivery to a minimum of 3 hours before
...
When composing the message, use the $ keyword to insert mission-specific links and values.
...
nearby at that time, making it the most optimal choice.
